...
function toUrlTemplate(pathName) {
  ...
  if(findProjectPath != null) {
    ...
    return `<url><loc>${`https://nizarbaihaqi.com/${filteredPath}`}</loc><lastmod>${data.date}</lastmod><changefreq>monthly</changefreq><priority>1.0</priority></url>`
  } else {
    return `<url><loc>${`https://nizarbaihaqi.com/${filteredPath}`}</loc><lastmod>${new Date().toISOString()}</lastmod><changefreq>monthly</changefreq><priority>1.0</priority></url>`
  }
}
...
async function generateSitemap() {
  ...
  const sitemap = `<?xml version="1.0" encoding="UTF-8" ?><urlset xmlns="http://sitemaps.org/schemas/sitemap/0.9">${allPageNames.map(toUrlTemplate).join('')}</urlset>`
